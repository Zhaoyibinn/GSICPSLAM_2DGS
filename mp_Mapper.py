...
class Mapper(SLAMParameters):
    ...
    def mapping(self):
                ...
                with torch.no_grad():
                    ...
                    if new_keyframe and self.rerun_viewer:
                        ...
                        if vis_normal:
                            ...
                            q = self.gaussians.get_rotation
                            q = q.cpu().detach()
                            q = np.concatenate([q[:, 1:], q[:, 0][:, np.newaxis]], axis=1)
                            # 注意，GS ICP SLAM,的实现里面的q的xyzw顺序和3DGS不一，参考https://github.com/Lab-of-AI-and-Robotics/GS_ICP_SLAM/issues/27
                            # GSICPSLAM采用的是(x， y， z， w),3DGS采用(w, x， y， z)，scipy也是
                            # q[11224] = np.array([0, 0, 0, 1])
                            # rot = R.from_quat(q)
                            normals = R.from_quat(q).as_matrix()[:, :, 2]
        ...
        if self.save_results:
            self.gaussians.save_ply(os.path.join(self.output_path + "/point_cloud/iteration_7000", "point_cloud.ply"))
            print("ply saved to "+ os.path.join(self.output_path + "/point_cloud/iteration_7000", "point_cloud.ply"))
