...
class GaussianModel(nn.Module):
    ...
    def rotation_matrix_from_normals(self,n1, n2):
        # 归一化
        ...
        # 计算旋转角度
        cos_theta = np.einsum('ij,ij->i', n1, n2)
        sin_theta = np.linalg.norm(u, axis=1, keepdims=True)
        ...
        # 构建旋转矩阵
        u = u / sin_theta
        K = np.array([[np.zeros_like(u[:,2]), -u[:,2], u[:,1]],
                      [u[:,2], np.zeros_like(u[:,2]), -u[:,0]],
                      [-u[:,1], u[:,0], np.zeros_like(u[:,2])]])
        K = np.transpose(K, (2, 0, 1))
        R = np.tile(np.eye(3), (u.shape[0] , 1, 1)) + np.sin(theta).reshape(-1,1,1) * K + (1 - np.cos(theta)).reshape(-1,1,1) * np.einsum('ijk,ikl->ijl', K, K)
    ...
    def create_from_pcd2_tensor(self, points, colors, rots_, scales_, z_vals_, trackable_idxs):
        ...
        z_vals = torch.clamp_min((z_vals_**1.5)*2., 1.).unsqueeze(-1).repeat(1,2)

        scales_withz = scales_
        scales = torch.log(scales_withz)
        rots = rots_
        
        opacities = inverse_sigmoid(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))

        # zyb标记，增加了计算法线并且旋转到计算的法线上
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(fused_point_cloud.cpu().detach())
        pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30))
        normals_np = np.array(pcd.normals)

        R_2normals = self.rotation_matrix_from_normals(np.tile(np.array([0,0,1]), (normals_np.shape[0], 1)),normals_np)
        # 这里旋转到z轴就是对的，可以改一下看变化
        # https://projects.markkellogg.org/threejs/demo_gaussian_splats_3d.php

        xyzw = Rotation.from_matrix(R_2normals).as_quat()
        wxyz = np.concatenate([xyzw[:, -1][:, np.newaxis], xyzw[:, :-1]], axis=1)
        nan_rows = np.where(np.isnan(wxyz).any(axis=1))[0]
        wxyz[nan_rows, :] = [1, 0, 0, 0]
        wxyz = torch.tensor(wxyz).cuda().float()
        # wxyz = torch.rand((fused_point_cloud.shape[0], 4), device="cuda")
        ...
        self._rotation = nn.Parameter(wxyz.requires_grad_(True))
    ...
    def add_from_pcd2_tensor(self, points, colors, rots_, scales_, z_vals_, trackable_idxs):
        ...
        self.new_rotation = nn.Parameter(wxyz.requires_grad_(True))
