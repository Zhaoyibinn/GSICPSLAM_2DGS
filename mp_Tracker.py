...
class Tracker(SLAMParameters):
    ...
    def save_poses(self):
        ...
        fname2pose = {}
        if not os.path.exists(dataset + '/sparse/0'):
            # 如果文件夹不存在，则创建它
            os.makedirs(dataset + '/sparse/0')
        with open(dataset + '/sparse/0/cameras.txt', 'w') as f:
        ...
        with open(dataset + '/sparse/0/images.txt', 'w') as f:
            for i in range(img_num):
                fname = str(i)
                if not (fname.endswith('.png') or fname.endswith('.jpg')):
                    fname += '.png'
                ...
                idx += 1
        with open(dataset + '/sparse/0/points3D.txt', 'w') as f:
            f.write('')
    ...
    def tracking(self):
        ...
        for ii in range(self.num_images):
            ...
            self.save_frame_points(points,ii)
            # GICP
            if self.iteration_images == 0:
                ...
                if if_tracking_keyframe:
                    ...
                    # zyb标记，这里手动加了一维
                    mean_values = target_scales.mean(axis=1, keepdims=True)
                    target_scales =  np.concatenate((target_scales, mean_values), axis=1)
